...
concept if no matching real product exists. It uses `meal_idea_llm_model`,
independent of the main pipeline model.
"""
...
def build_meal_idea_agent(*, model: Model | None = None) -> Agent[AgentDeps, list[MealConcept]]:
    ...
    return Agent(
        model if model is not None else settings.resolved_meal_idea_llm_model,
        deps_type=AgentDeps,
