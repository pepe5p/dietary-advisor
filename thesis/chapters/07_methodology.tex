...
The five scenarios were chosen to stress different parts of the system rather than to sample a population. \emph{regular} isolates baseline planning quality with no competing constraint. The two conflicting-request scenarios (\emph{lactose-intolerant-athlete-wants-cheesecake} and \emph{vegetarian-allergic}) test whether a hard exclusion survives a direct user request that would violate it, in two different forms (a single allergen against a high-energy target, and a diet pattern plus a second allergen at once). The two chronic-condition scenarios (\emph{diabetes-hypertension} and \emph{dyslipidemia-obesity}) test whether the system can satisfy interacting medical constraints, one without a session request and one with the conflicting-request pattern layered on top. Five is fewer than would be needed to support claims about dietary planning in general (\Cref{sec:results-threats} returns to this), but each additional scenario would multiply the grid cost and exceed the budget.
