\begin{tikzpicture}[
    ...
    box/.style={draw, rounded corners, align=center, text width=26mm, minimum height=9mm, inner sep=1.5mm, font=\scriptsize},
...
\node[title, above=8mm of o_mid] (o_title)
    {Open Food Facts\\ {\normalfont\scriptsize branded products}};
...
\node[title, above=8mm of u_mid] (u_title)
    {USDA FoodData Central\\ {\normalfont\scriptsize generic foods}};
...
\draw[arr] (tool.south) -| (o_frame.north);
\draw[arr] (tool.south) -| (u_frame.north);
\node[note, anchor=west, text width=38mm] at ($(tool.east) + (5mm, 0mm)$)
    {every query goes to both sources, searched concurrently};
...
\node[note, anchor=south, text width=72mm] at ($(result.north) + (0mm, 3mm)$)
    {no fusion across sources: each list keeps its own ranked order and its own namespaced codes};
\node[note, anchor=north, text width=84mm] at ($(result.south) + (0mm, -3mm)$)
    {each query caps its hits per source independently; a cap of zero skips that source};
