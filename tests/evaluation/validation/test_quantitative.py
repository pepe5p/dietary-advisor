...
def test_macro_errors_zero_when_totals_match_targets(mock_lookup: object) -> None:
    plan = agent_plan_rice_lunch(grams=200.0)
    totals = total_agent_meal_plan(plan, mock_lookup)  # type: ignore[arg-type]
    ...
    err = macro_errors(plan, targets, mock_lookup)  # type: ignore[arg-type]
    assert err.mae == 0.0
    assert err.mse == 0.0


def test_macro_errors_positive_when_off_target(mock_lookup: object) -> None:
    plan = agent_plan_rice_lunch(grams=400.0)
